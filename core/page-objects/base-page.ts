...
async checkMatchingElements(selector: By, matchingItem: string) { 
    ...
    const elementText = await element.getText(); expect(elementText).toMatch(matchingItem);
}
...
async scrollToElement(selector: By) {
    const element = await this.driver.findElement(selector);
    await this.driver.executeScript("arguments[0].scrollIntoView();", element);
}

async scroll(scrollLength: number) {
    await this.driver.executeScript(`window.scrollBy(0, ${scrollLength});`);
}
